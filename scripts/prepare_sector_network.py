...
def add_co2_tracking(n, costs, options, sequestration_potential_file=None):
    ...
    if options["regional_co2_sequestration_potential"]["enable"]:
        ...
        # map from location -> the actual sequestration bus name
        bus_map = dict(zip(spatial.co2.locations, sequestration_buses))

        e_nom_max = (
            e_nom_max
            .reindex(spatial.co2.locations)
            .fillna(0.0)
            .rename(index=bus_map)
            .reindex(sequestration_buses)     # enforce exact alignment and order
            .fillna(0.0)
        )
    ...
    for heat_system in (
        ...
        if enable_ates and heat_system == HeatSystem.URBAN_CENTRAL:
            ...
            n.add(
                ...
                #lifetime=costs.at["central geothermal heat source", "lifetime"],
                overnight_cost=250,     # €/MWh investment
                lifetime=30,
                discount_rate=0.07,
            )
...
def add_industry(
    ...
    print("len(spatial.oil.naphtha):", len(spatial.oil.naphtha))
    print("first naphtha names:", list(spatial.oil.naphtha)[:5])

    print("p_set_naphtha type:", type(p_set_naphtha))
    if hasattr(p_set_naphtha, "index"):
        print("p_set_naphtha index sample:", list(p_set_naphtha.index)[:5])
