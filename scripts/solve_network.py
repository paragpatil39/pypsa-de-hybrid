...
def apply_phaseout_existing_generators(
    n,
    carriers=("lignite", "coal", "nuclear"),
    country="DE",
):
    idx = n.generators.index[
        n.generators.bus.astype(str).str.startswith(country)
        & n.generators.carrier.isin(list(carriers))
    ]

    logger.info(f"Phase-out: matched {len(idx)} generators for {country} {list(carriers)}")
    logger.info(f"Phase-out: pre p_nom sum = {n.generators.loc[idx,'p_nom'].sum():.3f} MW")
    ...
    if "p_min_pu" in n.generators.columns:
        n.generators.loc[idx, "p_min_pu"] = 0.0
    if "active" in n.generators.columns:
        n.generators.loc[idx, "active"] = False

    logger.info(f"Phase-out: post p_nom sum = {n.generators.loc[idx,'p_nom'].sum():.3f} MW")
...
if __name__ == "__main__":
    ...
    logger.info("DEBUG sample generator index: %s", n.generators.index[:5].tolist())
    logger.info("DEBUG sample generator buses: %s", n.generators.bus.head().tolist())
    logger.info("DEBUG carriers present: %s", n.generators.carrier.unique())

    idx_by_bus = n.generators.index[n.generators.bus.astype(str).str.startswith("DE") & n.generators.carrier.isin(["coal","lignite","nuclear"])]
    idx_by_name = n.generators.index[(n.generators.index.to_series().str[:2] == "DE").values & n.generators.carrier.isin(["coal","lignite","nuclear"]).values]
    logger.info("DEBUG matches: by_bus=%d, by_name=%d", len(idx_by_bus), len(idx_by_name))

    apply_phaseout_existing_generators(
    n,
    carriers=("lignite", "coal", "nuclear"),
    country="DE",
    )

    logger.info("Post-phaseout DE coal/lignite/nuclear p_nom (sum):")
    logger.info(
        n.generators.loc[
            n.generators.bus.str.startswith("DE")
            & n.generators.carrier.isin(["coal", "lignite", "nuclear"]),
            "p_nom",
        ].sum()
    )
